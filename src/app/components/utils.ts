...
export function initMenuScene(container: PIXI.Container, toggleFullScreenCallback: () => void, startGameCallback: () => void) {
  ...
  const characterIcon = createSpriteFromImage(
    ...
    appConfig.constants.BASE_SIZE * 3,
    appConfig.constants.BASE_SIZE * 3,
    appConfig.constants.APP_WIDTH / 2 - appConfig.constants.BASE_SIZE * 1.5,
  ...
  const bestScoreText = createText(`Best Score: ${bestScore}`, textSize, "blue", appConfig.constants.APP_WIDTH / 2, appConfig.constants.APP_HEIGHT - appConfig.constants.BASE_SIZE * 2);
