...
export function initCommonAppElements(toggleFullScreenCallback: () => void) {
  ...
  const finishLine = createSpriteFromImage(
    ...
    appConfig.constants.APP_WIDTH - window.screen.width - appConfig.constants.BASE_SIZE * 3,
  ...
  return { app, background, mountains, clouds, character, ground, topBar, finishLine };
}
